            ...
            <div class="">
                ...
                <div class="img-btn-content">
                    <div class="flexImgContent">
                        @if(auth()->check() && auth()->user()->hasAnyRole(['Admin', 'Manager']) && auth()->user()->id !== $user->id)
    ...
    <div class="" style="width: 100%">
        ...
        @if($isEmployee)
            ...
            <!-- GRAPHIQUE DES PRESENCES -->
             <div class="attendance-calendrier">
                 <div class="attendance-section mt-10">
                    ...
                    @if($jourSuppExiste)
                    <div class="jourSuppContent">
                        <h2 class="suppTitle">Jours supplémentaires</h2>
                        @foreach($chartData as $data)
                            @if($data['jourSupplementaire'] > 0)
                            <div class="suppItem">
                                <span class="month">{{$data['month']}} : </span>
                                <span>{{$data['jourSupplementaire']}}</span>
                            </div>
                            @endif
                        @endforeach
                    </div>
                 ...
                 <div class="calendrier">
                    <h2 class="titreCalendrier">Calendrier hebdomadaire</h2>
                    @foreach($employee->calendrier_employees as $cal)
                        <div class="calendrierContent">
                            <p class="jour">{{ucfirst($cal->jour)}}</p> - {{$cal->horaire->name}} : <span>{{$cal->horaire->start_time->format('H:i')}} à {{$cal->horaire->end_time->format('H:i')}}</span>
                        </div>
                    @endforeach
                 </div>
             </div>
