<div class="">
    ...
    <h2 class="font-bold text-lg mb-2 text-gray-800">1. Informations de contact <span class="text-red-500 text-sm">*</span></h2>
    ...
    <div class="modePayement">
        <h2 class="font-bold text-lg mb-1 mt-5 text-gray-800">2. Mode de payement <span class="text-red-500 text-sm">*</span></h2>
    ...
    <div class="modeDeLivraison">
        ...
        <h2 class="font-bold text-lg mb-2 mt-5 text-gray-800">3. Mode de livraison <span class="text-red-500 text-sm">*</span></h2>
        ...
        <!-- LAISSER UNE NOTE -->
        <div class="modePayement">
            <h2 class="font-bold text-lg mb-2 mt-5 text-gray-800">5. Laissez une note </h2>
            <textarea wire:model="note" class="modesPayementContent" placeholder="Pas de piment"></textarea>
        </div>
    </div>

    @if($commandeCreee == false)
        <p class="btnRevenir mt-5" wire:click="retourCart">
            <x-icon name="fleche-left" fill="#727171" />
            <span>Revenir en arrière</span>
        </p>
        <div class="flex gap-3 items-center mt-5 mb-4">
            <button type="submit" class="btnSubmitCmd">Valider la commande</button>
            <div x-data = "{confirmCancelCmd: false}">
                <p @click="confirmCancelCmd = true"  type="button" class="btnAnnuler">Annuler</p>

                <div x-show="confirmCancelCmd" x-cloak class="confirmationBox" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-200"  x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95">
                    <div class="confirmationContent">
                        <h3 class="text-lg font-bold mb-4">Annuler la commande</h3>
                        <p class="mb-2">Êtes-vous sûr de vouloir annuler votre commande ?</p>
                        <p class="mb-2">Cette action est irréversible</p>
                        <div class="flex justify-end gap-2">
                            <p @click="confirmCancelCmd = false" class="btnAnnulerCmd cursor-pointer">
                                Revenir
                            </p>
                            <p type="button" wire:click="annulerCmd" @click="confirmCancelCmd = false" class="btnConfirmerCmd" style="background: #8b0f0f">
                                Annuler la commande
                            </p>
                        </div>
                    </div>
        ...
        <div class="text-[15px] text-gray-900">
            <p>
                En cliquant sur <strong>Valider</strong>, vous confirmez les informations saisies.
            </p>
            <p>Cliquez sur <strong>Annuler</strong> pour annuler la commande</p>
        </div>
    @else
