@foreach($productSuggeres as $pSuggere)
    ...
    <div class="bg-[#fff] swiper-slide product-slide relative w-[220px] rounded-md shadow-md shadow-black/10">
        <div class="wishlist absolute top-2 left-2 z-10 ">
            @auth()
            <button wire:click="addToWishlist({{$pSuggere->id}})" class="coeur-vide">
                @if(Cart::instance('wishlist')->content()->where('id', $pSuggere->id)->count() > 0)
        ...
        <div class="">
            <a href="{{route('product.view', ['product' => $pSuggere])}}" class="oeil bg-[#F0F0F0] md:py-2 md:px-2 px-1 py-1 rounded-full absolute top-2 right-2">
        ...
        <div class="content relative mt-[10px]">
            <div class="">
                <p class="text-center font-semibold md:text-lg ">{{$pSuggere->name}}</p>
                <p class="hidden md:block text-center font-medium text-sm text-gray-400">{{$pSuggere->sousCategory->name}}</p>
            </div>
            <div class="flex gap-2 justify-center font-semibold text-[13px]">
                <span class="money price">
                @if($pSuggere->sale_price)
                    <s class="text-gray-400">{{str_replace(',','.',number_format($pSuggere->regular_price, 0))}}</s> <span class="text-[--color2-yellow]">{{str_replace(',','.',number_format($pSuggere->sale_price, 0))}} GNF</span>
                @else
                    <p class="text-[--color2-yellow]">
                    {{str_replace(',','.',number_format($pSuggere->regular_price, 0))}} GNF
                    </p>
                @endif
                </span>
            </div>
            <div class="flex justify-center mt-2 gap-[6px]">
                ...
                @auth()
                    @if(Cart::instance('cart')->content()->where('id', $pSuggere->id)->count() > 0)
                    ...
                    @else
                    <button wire:click="addToCart({{$pSuggere->id}})" class="btnPanier text-center border-[1px] border-[--color2-yellow] text-green-800 px-2 py-[2px] rounded-full group text-xs hover:text-white">
                ...
                <div class="">
                    @auth()
                    <a href="{{route('buy.now', ['product'=>$pSuggere])}}" class="group-hover:text-green-800 btnPanier text-center border-[1px] border-green-800 bg-green-800 py-[2px] md:py-1 px-2 text-white hover:text-green-800 rounded-full group hover:bg-white text-xs">Commander</a>
                    @endauth
